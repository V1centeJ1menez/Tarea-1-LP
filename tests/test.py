            ...
            while pos < len(linea):
                ...
                for patron, tipo in self.patrones_compilados:
                    ...
                    if match:
                        ...
                        break
                if not match:
                    # Capturar el carácter que no coincide y manejarlo
                    caracter_no_conocido = linea[pos]
                    self.tokens.append(Token('MISMATCH', caracter_no_conocido, numero_linea))
                    print(f"Token MISMATCH: '{caracter_no_conocido}' en línea {numero_linea}")
                    pos += 1  # Avanzar un carácter
...
codigo_fuente = [
    "DEFINE x = 5",
    "if (x > 0) { MOSTRAR # Hello World # }",
    "ELSE { DP $VAR }",
    "Invalid$Character"  # Ejemplo de un carácter no válido
]
